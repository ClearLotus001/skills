...
## 输入探查（可选）
当需要了解输入文件结构（sheet 名、列头）时，使用 `--scan` 模式：
```bash
python scripts/run_validator.py \
  ...
  --scan
```
- 扫描结果写入 `<out>/_scan.json`，包含文件名、sheet 列表、每个 sheet 的列头。
- 扫描日志写入 `<out>/_run.log`。
- 执行后读取 `_scan.json` 获取文件结构信息，据此编写 `rules.json`。
...
## 执行后获取结果
执行完成后，通过以下方式获取结果（**不要依赖终端输出**）：
1. 读取 `<out>/_run.log` 获取执行日志（包含进度、警告、错误信息）。
2. 读取 `<out>/result.json` 获取完整校验结果（issues 列表、统计摘要）。
3. 打开 `<out>/report.html` 查看交互式报告。
...
- `datasets`：文件/工作表存在性检查（支持 `file` 与 `file_pattern`）。
  ...
  - **文件身份追踪**：每个 issue 附带 `file_path`（完整路径）和 `file_sha256`（SHA-256 指纹），CSV 报告中同步展示。
